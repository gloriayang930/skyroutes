...
def new_route(start_point=None, end_point=None):
  ...
  if shortest_route:
    shortest_route_string = '\n'.join(shortest_route) #Join items in a list
...
def get_route(start_point, end_point):
  ...
  for start_station in start_stations:
    for end_station in end_stations:
      ...
      if route:
        routes.append(route)
  shortest_route = min(routes, key=len) #Tricky!
  ...
  for station_under_construction in stations_under_construction:
    for current_station, neighboring_stations in vc_metro.items():
      if current_station != station_under_construction:
        updated_metro[current_station] -= set(stations_under_construction) # Remove the connections between current and constructing stations
      else:
